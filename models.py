...
class Models():
    ...
    def bins_vgg16(opt = 1):
        ...
        if (opt == 1):
            ...
            x1 = layers.Dense(N_BINS, activation='softmax', input_shape=(None, 25088), trainable=True)(preds)
            x2 = layers.Dense(N_BINS, activation='softmax', input_shape=(None, 25088), trainable=True)(preds)
            x3 = layers.Dense(N_BINS, activation='softmax', input_shape=(None, 25088), trainable=True)(preds)
            x4 = layers.Dense(N_BINS, activation='softmax', input_shape=(None, 25088), trainable=True)(preds)
            x5 = layers.Dense(N_BINS, activation='softmax', input_shape=(None, 25088), trainable=True)(preds)
            x6 = layers.Dense(N_BINS, activation='softmax', input_shape=(None, 25088), trainable=True)(preds)
            x7 = layers.Dense(N_BINS, activation='softmax', input_shape=(None, 25088), trainable=True)(preds)
            x8 = layers.Dense(N_BINS, activation='softmax', input_shape=(None, 25088), trainable=True)(preds)
            x9 = layers.Dense(N_BINS, activation='softmax', input_shape=(None, 25088), trainable=True)(preds)
            x10 = layers.Dense(N_BINS, activation='softmax', input_shape=(None, 25088), trainable=True)(preds)
            preds = Concatenate(axis = 1)([x1, x2, x3, x4, x5, x6, x7, x8, x9, x10])
